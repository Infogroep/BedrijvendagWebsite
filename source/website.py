...
def static_company_page(name):
    ...
    if(request.session.get('logged_in') == name):
        location = request.forms.get('location').encode('UTF-8')
        slogan = request.forms.get('slogan').encode('UTF-8')
        why = request.forms.get('why').encode('UTF-8')
        field = request.forms.get('schooling').encode('UTF-8')
        develop = request.forms.get('develop').encode('UTF-8')
        NL = bool(request.forms.get('NL')).encode('UTF-8')
        FR = bool(request.forms.get('FR')).encode('UTF-8')
        DE = bool(request.forms.get('DE')).encode('UTF-8')
        ENG = bool(request.forms.get('ENG')).encode('UTF-8')
        JOBS = bool(request.forms.get('JOBS')).encode('UTF-8')
        SJOBS = bool(request.forms.get('SJOBS')).encode('UTF-8')
        STAGE = bool(request.forms.get('STAGE')).encode('UTF-8')
        FY = bool(request.forms.get('FY')).encode('UTF-8')
        CH = bool(request.forms.get('CH')).encode('UTF-8')
        WI = bool(request.forms.get('WI')).encode('UTF-8')
        BIO = bool(request.forms.get('BIO')).encode('UTF-8')
        CW = bool(request.forms.get('CW')).encode('UTF-8')
        BIN = bool(request.forms.get('BIN')).encode('UTF-8')
        GEO = bool(request.forms.get('GEO')).encode('UTF-8')
