...
def static_company_page(name):
    ...
    if(request.session.get('logged_in') == name):
        location = request.forms.get('location').decode('UTF-8')
        slogan = request.forms.get('slogan').decode('UTF-8')
        why = request.forms.get('why').decode('UTF-8')
        field = request.forms.get('schooling').decode('UTF-8')
        develop = request.forms.get('develop').decode('UTF-8')
        NL = bool(request.forms.get('NL')).decode('UTF-8')
        FR = bool(request.forms.get('FR')).decode('UTF-8')
        DE = bool(request.forms.get('DE')).decode('UTF-8')
        ENG = bool(request.forms.get('ENG')).decode('UTF-8')
        JOBS = bool(request.forms.get('JOBS')).decode('UTF-8')
        SJOBS = bool(request.forms.get('SJOBS')).decode('UTF-8')
        STAGE = bool(request.forms.get('STAGE')).decode('UTF-8')
        FY = bool(request.forms.get('FY')).decode('UTF-8')
        CH = bool(request.forms.get('CH')).decode('UTF-8')
        WI = bool(request.forms.get('WI')).decode('UTF-8')
        BIO = bool(request.forms.get('BIO')).decode('UTF-8')
        CW = bool(request.forms.get('CW')).decode('UTF-8')
        BIN = bool(request.forms.get('BIN')).decode('UTF-8')
        GEO = bool(request.forms.get('GEO')).decode('UTF-8')
